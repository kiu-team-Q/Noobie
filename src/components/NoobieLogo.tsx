...
export const NoobieLogo = ({ className = "", textSize = "text-5xl" }: NooglesLogoProps) => {
  ...
  return (
    <div ref={containerRef} className={`inline-flex items-center gap-0 ${className}`}>
      <span className={`${textSize} font-playfair font-bold tracking-tight text-foreground`}>N</span>
      ...
      <span className={`${textSize} font-playfair font-bold tracking-tight text-foreground`}>bie</span>
    </div>
  );
};
